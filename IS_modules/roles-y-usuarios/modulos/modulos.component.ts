...
export class ModulosComponent implements OnInit {
  ...
  constructor(public rolesMensajesService: RolesMensajesService, private servicioAccesos: WeborbAccesosService,
    ...
    this.inicializarDropdownIconos();
  }
  ...
  private inicializarDropdownIconos(): void {
    this.dropdownIconos = [];
    const IconosSeperados = this.IconosPrimeng.split(', ');
    for (const Icono of IconosSeperados) {
      this.dropdownIconos.push({label: Icono, value: Icono});
    }
  }
  ...
  private obtenerModulos(): void {
    ...
    setTimeout(() => {
        Promise.resolve(this.servicioAccesos.seleccionarModulosPorProyecto(this.nombreProyecto)).then(resultado => {
          this.tableLoader = false;
          if (resultado != null) {
            this.modulosDelSistema = resultado;
          }
        }).catch(() => {
  ...
  private guardarModulo(): void {
    this.cargarSpinner = true;
    const NuevoModulo: Modulo = {
      cvemodulo: this.inputClave,
      posmodulo: this.inputPosicion,
      descmodulo: this.inputDescripcion,
      cveproy: this.nombreProyecto,
      rutaapps: this.inputRuta,
      nameapp: '',
      paramapp: '',
      icono: this.inputIcono
    };
    setTimeout(() => {
        Promise.resolve(this.servicioAccesos.guardarModulo(NuevoModulo)).then(resultado => {
          this.cargarSpinner = false;
          if (resultado > 0) {
            this.rolesMensajesService.showTempMessage(RolesMensajesService.Pgrowl_Succes, 'Guardado', 'El ' +
              'módulo fue guardado exitosamente.');
            this.esModoNuevo = false;
            this.obtenerModulos();
          } else {
            this.rolesMensajesService.showTempMessage(RolesMensajesService.Pgrowl_Error, 'Error', 'Ha ' +
              'ocurrido un error del sistema, favor de llamar a informática (ext. 4223).');
          }
        }).catch(() => {
          console.log('Error en obtenerRoles');
        });
  ...
  /*--- Eventos ---*/
  clicEditarModulo(): void {
    this.inputClave = this.moduloSeleccionado.cvemodulo;
    this.inputPosicion = this.moduloSeleccionado.posmodulo;
    this.inputDescripcion = this.moduloSeleccionado.descmodulo;
    this.inputIcono = this.moduloSeleccionado.icono;
    this.inputRuta = (this.moduloSeleccionado.rutaapps != null) ? this.moduloSeleccionado.rutaapps : '';
    this.esEdicion = true;
    this.esModoNuevo = true;
  }

  clicEliminarModulo(): void {
    this.servicioDeConfirmacion.confirm({
      message: '¿Realmente desea borrar este módulo?',
      accept: () => {
        this.tableLoader = true;
        setTimeout(() => {
            Promise.resolve(this.servicioAccesos.borrarModulo(this.nombreProyecto, this.moduloSeleccionado.cvemodulo))
              .then(resultado => {
                this.tableLoader = false;
                if (resultado > 0) {
                  this.rolesMensajesService.showTempMessage(RolesMensajesService.Pgrowl_Succes, 'Eliminado', 'El ' +
                    'módulo fue eliminado exitosamente.');
                  this.obtenerModulos();
                } else {
                  this.rolesMensajesService.showTempMessage(RolesMensajesService.Pgrowl_Error, 'Error', 'Ha ' +
                    'ocurrido un error del sistema, favor de llamar a informática (ext. 4223).');
  ...
  clicValidarModulo(): void {
    // Checar si la posición es un número
    if (this.esNumero(this.inputPosicion)) {
      if (!this.esEdicion) {
        this.validarExisteModulo();
      } else {
        this.guardarModulo();
      }
    } else {
      this.rolesMensajesService.showTempMessage(RolesMensajesService.Pgrowl_Error, 'Error', 'La posición ' +
        'debe de ser un número entero o flotante.');
    }
  }
  private validarExisteModulo(): void {
    // Checar si ya existe un módulo con esa clave en el proyecto
    this.cargarSpinner = true;
    setTimeout(() => {
        Promise.resolve(this.servicioAccesos.seleccionarModulo(this.nombreProyecto, this.inputClave)).
        then(resultado => {
          this.cargarSpinner = false;
          if (resultado == null) {
            this.guardarModulo();
          } else {
            this.rolesMensajesService.showTempMessage(RolesMensajesService.Pgrowl_Error, 'Error', 'La ' +
              'clave ya existe en otro módulo dentro de este proyecto ' + this.nombreProyecto);
          }
        }).catch(function (e) {
          console.log('Error en obtenerRoles');
        });
